...
func (store *blockchainDatabase) putBlockInternal(hash common.Hash, header *types.BlockHeader, body *blockBody, td *big.Int, isHead bool) error {
	...
	if isHead {
		...
		if err == nil {
			...
			if err == nil {
				store.batchDeleteIndices(batch, oldHash, oldBlock.Transactions)
			}
		}
...
func (store *blockchainDatabase) AddIndices(block *types.Block) error {
	batch := store.db.NewBatch()
	store.batchAddIndices(batch, block.HeaderHash, block.Transactions)
	return batch.Commit()
}

func (store *blockchainDatabase) batchAddIndices(batch database.Batch, blockHash common.Hash, txs []*types.Transaction) {
	for i, tx := range txs {
...
func (store *blockchainDatabase) DeleteIndices(block *types.Block) error {
	...
	if err := store.batchDeleteIndices(batch, block.HeaderHash, block.Transactions); err != nil {
		return err
	}
...
func (store *blockchainDatabase) batchDeleteIndices(batch database.Batch, blockHash common.Hash, txs []*types.Transaction) error {
	for _, tx := range txs {
