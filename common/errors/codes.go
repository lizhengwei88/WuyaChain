...
var constErrors = map[ErrorCode]error{
	ErrDecrypt:                newWuyaError(ErrDecrypt, "could not decrypt key with given passphrase"),
	ErrEmptyAuthKey:           newWuyaError(ErrEmptyAuthKey, "encryption auth key could not be empty"),
	ErrPasswordRepeatMismatch: newWuyaError(ErrPasswordRepeatMismatch, "repeat password is not equal to orignal one"),
}
